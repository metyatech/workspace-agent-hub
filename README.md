...
- For existing repositories, Manager delivers approved changes by calling
  ...
  task worktree before asking the user, deterministically retries a
  non-fast-forward push if the target branch advanced during deliver, and only
  preserves that worktree as a paused recovery worktree if automatic recovery
  is exhausted. If the recovery router itself fails, Hub now surfaces that
  actual recovery runtime failure instead of collapsing it into a generic parse
  error.
