...
function Invoke-HubSessionsRequest {
    ...
    return Invoke-WebRequest -Uri "http://127.0.0.1:$PortNumber/api/sessions?includeArchived=true" -Headers $headers -TimeoutSec 3 -UseBasicParsing -ErrorAction Stop
}
...
try {
    ...
    $firstResponse = Invoke-HubSessionsRequest -PortNumber $firstPort
    ...
    do {
        try {
            $response = Invoke-HubSessionsRequest -PortNumber $firstPort
    ...
    $replacementResponse = Invoke-HubSessionsRequest -PortNumber $replacementPort -Token 'replacement-token'
